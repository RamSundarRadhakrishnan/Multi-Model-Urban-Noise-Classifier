...
def plot_fusion_weight_sensitivity(sweep_path, output_path):
    df = pd.read_csv(sweep_path)

    fusion = df[df["variant"].str.startswith("late_fusion_alpha_")].copy()
    fusion["alpha"] = fusion["variant"].str.extract(r"([0-9.]+)$").astype(float)
    fusion = fusion.sort_values("alpha")
    ...
    ax.axvline(0.5, color="#2E8B57", linestyle="--", linewidth=1.2)
    ax.annotate(
        r"Reported $\alpha=0.5$",
        xy=(0.5, 39.2),
        xytext=(0.515, 39.2),
        color="#2E8B57",
        fontsize=9,
    ...
    ax.set_xlabel(r"Audio weight $\alpha$ in late fusion")
    ax.set_ylabel("Score (%)")
    ax.set_xticks(fusion["alpha"])
    ax.set_ylim(35, 50)
    ax.grid(axis="y", linestyle="--", alpha=0.35)
    ax.set_axisbelow(True)
...
def main():
    ...
    plot_fusion_weight_sensitivity(
        Path("vggsound_alpha_sweep_summary.csv"),
        output_dir / "vggsound_fusion_weight_sensitivity.png",
    )
